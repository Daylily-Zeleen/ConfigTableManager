...
@tool
extends "res://addons/config_table_manager.daylily-zeleen/scripts/generate_modifier.gd"
...
func _modify_data(data: Array[Dictionary]) -> Array[Dictionary]:
	const generated_name = "generated_data"
	# 检查是否有该行
	for d in data:
	...
	# 添加生成数据
	data.push_back({id = data.size(), name = generated_name, description = "该行由生成修改器生成。"})
	return data
